...
public:
  ...
  void PushBack(Image::UniquePtr & ptr)
  {
    ...
    // RCLCPP_INFO(rclcpp::get_logger("laser_line_center"),"%s received a img", _node -> get_name());//todo
    ...
    while (rclcpp::ok())
    {
      ...
      if (_deq.empty() == false)
      {
        ...
        else
        {
          ...
          _node->Publish(line);
          // RCLCPP_INFO(rclcpp::get_logger("laser_line_center"),"%s published a line",_node -> get_name());//todo
        }
      }
      else
      {
...
LaserLineCenter::LaserLineCenter(const rclcpp::NodeOptions & options) : Node("laser_line_center_node", options)
{
