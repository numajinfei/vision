...
void GpioRaspberry::_GpioGrabImage(void)
{
  ...
  if(rclcpp::ok() && _clientDisableSaveL->service_is_ready() && _clientDisableSaveR->service_is_ready())
  {
    ...
    while(loop && (num < 600)) //TODO： 300 待确定单次测量最大所需图片数
    {        
      if(rclcpp::ok())
      {
      	
        std::this_thread::sleep_for(8ms); //=1000/帧率
        Toggle(_lineBaslerTrigger);
        std::this_thread::sleep_for(8ms); //=1000/帧率
        Toggle(_lineBaslerTrigger);
        //std::cout<<"toggle"<<std::endl;
        num ++;
        // std::cout<< "[" << this->get_name() << "]" << ": number: " << num << std::endl;
      }            
      else
    ...
    RCLCPP_INFO(rclcpp::get_logger(this->get_name()), "GrabImage' num is %d", num);
    //todo
        std::ofstream of("/home/ubuntu/http_server/gpio_frameID.txt", std::ofstream::out | std::ofstream::app);
        if(!of.is_open())
        {
            throw std::runtime_error("failed to open gpio_frameID.txt");
        }

        of << ++serial_number << ". trigger_times: " << num << "\n";
...
RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "EnableSave is ready...");
    std::this_thread::sleep_for(5ms);

    Toggle(_lineBaslerTrigger);
RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "GPIO is toggle...");	
	///*
	while(loop && (num < 10)) //TODO： 300 待确定单次测量最大所需图片数
    {        
      if(rclcpp::ok())
      {
