#pragma once
#ifndef _MASONRY_PERPENDICULARITY_NODE_H
#define _MASONRY_PERPENDICULARITY_NODE_H
...
class MasonryPerpendicularityNode : public rclcpp::Node
{
    public:
        MasonryPerpendicularityNode(const rclcpp::NodeOptions& option = rclcpp::NodeOptions());
        ~MasonryPerpendicularityNode();
    ...
    private:
        ...
        void PubResult(const std::vector<float>& result);

        void PubStatus(const std::string& status);

    private:
        MasonryPerpendicularity _masonryPerpendicularity;
...
#endif //_MASONRY_PERPENDICULARITY_NODE_H