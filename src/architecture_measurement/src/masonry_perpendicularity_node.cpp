#include "architecture_measurement/masonry_perpendicularity_node.hpp"

namespace am
{

MasonryPerpendicularityNode::MasonryPerpendicularityNode(const rclcpp::NodeOptions& option) : rclcpp::Node("masonry_perpendicularity_node", option)
{
    _initThread = std::thread(&MasonryPerpendicularityNode::Init, this);
}

MasonryPerpendicularityNode::~MasonryPerpendicularityNode() 
{
    try
    {
        _initThread.join();
        ...
        RCLCPP_INFO(this->get_logger(),"MasonryPerpendicularityNode destroyed successfully.");
    }
    catch(const std::exception& e)
    {
        RCLCPP_INFO(this->get_logger(), "Exception in MasonryPerpendicularityNode destruction: %s", e.what());
        rclcpp::shutdown();
    }
    catch(...)
    {
        RCLCPP_INFO(this->get_logger(),"Exception in MasonryPerpendicularityNode destruction: unknown");
        rclcpp::shutdown();
    }
}


void MasonryPerpendicularityNode::Init() 
{
    try
    {            
        _status = -1;
        ...
        _subPointCloud = this->create_subscription<shared_interfaces::msg::PointCloudC>(
            _subPointCloudName, 
            1, 
            std::bind(&MasonryPerpendicularityNode::SubPointCloud, this, std::placeholders::_1));

        _subImage = this->create_subscription<shared_interfaces::msg::ImageC>(
            _subImageName,
            1,
            std::bind(&MasonryPerpendicularityNode::SubImage, this, std::placeholders::_1));

        _subInclinometer = this->create_subscription<shared_interfaces::msg::Inclinometer>(
            _subInclinometerName, 
            5, 
            std::bind(&MasonryPerpendicularityNode::SubInclinometer, this, std::placeholders::_1));
        ...
        RCLCPP_INFO(this->get_logger(), "MasonryPerpendicularityNode initialized successfully.");
        
    }
    catch(const std::exception& e)
    {
        RCLCPP_ERROR(this->get_logger(), "Exception in MasonryPerpendicularityNode initializer: %s", e.what());
        rclcpp::shutdown();
    }
    catch(...)
    {
        RCLCPP_ERROR(this->get_logger(),"Exception in MasonryPerpendicularityNode initializer: unknown");
        rclcpp::shutdown();
    }
}


void MasonryPerpendicularityNode::InitializeParameters()
{
    ...
    this->declare_parameter<std::string>("fileName","fileName");
}

void MasonryPerpendicularityNode::UpdateParameters()
{
    ...
    _masonryPerpendicularity.SetI_R_C(_I_R_C);
    _masonryPerpendicularity.SetFileName(_fileName);
}

void MasonryPerpendicularityNode::SubPointCloud(shared_interfaces::msg::PointCloudC::UniquePtr pointCloudCPtr)
{
    if(_status < 0)
    {
    ...
    std::vector<float> result = _masonryPerpendicularity.Calculate(pointCloudPtr, pointCloudFilterPtr, indicesFilterPtr, 
...
void MasonryPerpendicularityNode::SubImage(shared_interfaces::msg::ImageC::UniquePtr ptr)
{
    // if(ptr->engineering == "Masonry" && ptr->option == "WallVertical")
    // {
        RCLCPP_INFO(this->get_logger(), "sub an image form mortar joint node");
        // _combinationDate._image = cv::Mat(ptr->image.height, ptr->image.width, CV_8UC1, ptr->image.data.data());
        // _masonryPerpendicularity.SetImageWidth(ptr->image.width);
        // _masonryPerpendicularity.SetImageHeight(ptr->image.height);
        // _combinationDate._imageID = ptr->id;
    // }
}

void MasonryPerpendicularityNode::SubInclinometer(shared_interfaces::msg::Inclinometer::UniquePtr ptr)
{
    if(_status < 0)
        return;
    ...
    _masonryPerpendicularity.SetAngleXY(angleX, angleY);
}

void MasonryPerpendicularityNode::PubResult(const std::vector<float>& result)
{
    if(_status < 0)
    {
    ...
    RCLCPP_INFO(this->get_logger(), "publish result: %f.", result[0]);
}

void MasonryPerpendicularityNode::PubStatus(const std::string& status)
{
    if(_status < 0)
    {
...
RCLCPP_COMPONENTS_REGISTER_NODE(am::MasonryPerpendicularityNode)