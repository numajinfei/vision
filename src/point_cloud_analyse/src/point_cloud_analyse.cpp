...
void Save(pcl::PointCloud<pcl::Normal>::Ptr normal, const std::string& fileName)
{
    ...
    {
        ofile << p.normal[0] * 1000 << ' ' << p.normal[1] * 1000 << ' ' << p.normal[2] * 1000 << ' ' << p.curvature * 1000 << '\n';
    }
}
...
void Save(pcl::PointCloud<pcl::PointXYZI>::Ptr cloud, const std::string& fileName)
{
    ...
    {
        ofile << p.x * 1000 << ' ' << p.y * 1000 << ' ' << p.z * 1000 << ' ' << p.intensity * 1000 << '\n';
    }
}
...
{
public:
    _Impl(PointCloudAnalyse* ptr) : _node(ptr)
    {
    }
    ...
    pcl::ModelCoefficients Plane(pcl::PointCloud<pcl::PointXYZI>::Ptr cloud, pcl::PointIndices& inliers)
    {
        ...
        seg.setDistanceThreshold(0.02);//TODO
    ...
    void Segment(pcl::PointCloud<pcl::PointXYZI>::Ptr cloud, std::vector<pcl::PointIndices>& regions)
    {
        ...
        vg.setLeafSize(0.01, 0.2, 0.01);//TODO
        ...
        ne.setKSearch(25);//TODO yzl
        ne.compute(*normal);

        RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Segment normal size: %d", normal->size());
        pcl::io::savePCDFileASCII ("/home/ubuntu/http_server/cloud.pcd", *cloud);
        //Save(normal, "/home/ubuntu/http_server/normal.txt");

        //Region grow
        pcl::RegionGrowing<pcl::PointXYZI, pcl::Normal> rg;
        rg.setMinClusterSize(1000);//TODO
        rg.setMaxClusterSize(1000000);//TODO
        rg.setSearchMethod(tree);
        rg.setNumberOfNeighbours(25);//TODO yzl
        rg.setInputCloud(cloud);
        rg.setInputNormals(normal);
        rg.setSmoothnessThreshold(5.0 / 180.0 * M_PI);//TODO
        rg.setCurvatureThreshold (1.0);//TODO
        rg.extract(regions);

        //Sort region
        for(size_t i = 0; i < regions.size(); ++i)
        {
            //RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Segment region[%d] size: %d",i, regions[i].indices.size());
            auto& ri = regions[i];
        ...
        //RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Segment region size: %d", regions.size());
        //RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Segment: %d", M_PI);
    }
    ...
    std::pair<double, double> Statistic(pcl::PointCloud<pcl::PointXYZI>::Ptr cloud)
    {
        ...
        for(const auto& p : *cloud)
        {
            v.push_back(p.intensity);
        }
        auto sum = std::accumulate(v.begin(), v.end(), 0.0);
        auto mean = sum / v.size();
        std::vector<double> diff(v.size());
        std::transform(v.begin(), v.end(), diff.begin(), [mean](double x) { return x - mean; });
        auto dev = std::sqrt(std::inner_product(diff.begin(), diff.end(), diff.begin(), 0.0) / v.size());
    ...
    std::vector<double> VirtualRuler(pcl::PointCloud<pcl::PointXYZI>::Ptr cloud)
    {
        ...
        const double P[L][4] = {{1, 0, 0, 0}, {0, 0, 1, 0}, {std::sqrt(0.5), 0, std::sqrt(0.5), 0}, {std::sqrt(0.5), 0, -std::sqrt(0.5), 0}};
        ...
        return {s0.first * 1000, s0.second * 1000, s1.first * 1000, s1.second * 1000, s2.first * 1000, s2.second * 1000, s3.first * 1000, s3.second * 1000};
    }

    std::vector<double> CalculatePlaneFlat(pcl::PointCloud<pcl::PointXYZI>::Ptr cloud)
    {
        //Save(cloud, "cloud.txt");
        ...
        RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "CalculatePlaneFlat: Segment region size: %d", regions.size());
        if(regions.empty())
        {
    ...
    std::vector<double> CalculateIncludedAngle(pcl::PointCloud<pcl::PointXYZI>::Ptr cloud)
    {
        ...
        std::vector<pcl::PointIndices> regions;
        Segment(cloud, regions);
        RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "CalculateIncludedAngle: Segment region size: %d", regions.size());
        ...
        *cloud0 += *cloud1;
        ...
        return {acos(c0.values[0] * c1.values[0] + c0.values[1] * c1.values[1] + c0.values[2] * c1.values[2]) / M_PI * 180.};
    }
...
PointCloudAnalyse::~PointCloudAnalyse()
{
    ...
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "point_cloud_analyse destroyed successfully");
}

void PointCloudAnalyse::_Init() try
{
    ...
    _sub = this->create_subscription<sensor_msgs::msg::PointCloud2>(_subName, 1, std::bind(&PointCloudAnalyse::_Sub, this, std::placeholders::_1));

    _subRpy = this->create_subscription<shared_interfaces::msg::RollPitchYaw>(_subRpyName, 10, [this](shared_interfaces::msg::RollPitchYaw::UniquePtr ptr){
        _roll = ptr->roll;
        _pitch = ptr->pitch;
        _yaw = ptr->yaw;
    });

    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "point_cloud_analyse initialized successfully");
}
catch(const std::exception& e)
{
    RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "Exception in point_cloud_analyse initializer: %s", e.what());
}
catch(...)
{
    RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "Exception in point_cloud_analyse initializer: unknown");
}

void PointCloudAnalyse::_Sub(sensor_msgs::msg::PointCloud2::UniquePtr ptr) try
{
    _UpdateParameters();
...
catch(const std::exception& e)
{
    auto msg = std::make_unique<shared_interfaces::msg::Float64Array>();
    _pubResult->publish(std::move(msg));
    RCLCPP_WARN(rclcpp::get_logger("rclcpp"), "Exception in point_cloud_analyse subscription: %s", e.what());
}
catch(...)
{
    auto msg = std::make_unique<shared_interfaces::msg::Float64Array>();
    _pubResult->publish(std::move(msg));
    RCLCPP_WARN(rclcpp::get_logger("rclcpp"), "Exception in point_cloud_analyse subscription: unknown");
}
